...
class PentaFusion_block(nn.Module):
    ...
    def forward(self, rgb, lidar):
        ...
        # fusion
        fuse = self.residual(
            torch.cat([rgb, lidar, rgb_pool_out, lidar_pool_out, bp], 1))
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer,
        ...
        self.bifusion = PentaFusion_block(ch_1=n_embd, ch_2=n_embd, r_1=n_embd //
                                          64, r_2=n_embd//64, ch_int=n_embd, ch_out=n_embd, drop_rate=embd_pdrop/2)

        # positional embedding parameter (learnable), image + lidar
        self.pos_emb = nn.Parameter(torch.zeros(
            1, seq_len * vert_anchors * horz_anchors, n_embd))
    ...
    def forward(self, image_tensor, lidar_tensor, velocity):
        ...
        x = x.view(bz, self.seq_len, self.vert_anchors,
                   self.horz_anchors, self.n_embd)
        x = x.permute(0, 1, 4, 2, 3).contiguous()  # same as token_embeddings

        fusion_tensor_out = x.view(
...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, velocity):
        ...
        if self.image_encoder.normalize:
            image_list = [normalize_imagenet(image_input)
        ...
        image_tensor = torch.stack(image_list, dim=1).view(
            bz * self.config.n_views * self.config.seq_len, img_channel, h, w)
        lidar_tensor = torch.stack(lidar_list, dim=1).view(
            bz * self.config.seq_len, lidar_channel, h, w)
        ...
        image_features_layer1 = self.rgb_se_layer1(image_features_layer1) * image_features_layer1
        lidar_features_layer1 = self.lidar_se_layer1(lidar_features_layer1) * lidar_features_layer1
        image_features_layer1 = F.interpolate(
            image_features_layer1, scale_factor=8, mode='bilinear')
        lidar_features_layer1 = F.interpolate(
            lidar_features_layer1, scale_factor=8, mode='bilinear')
        ...
        image_features_layer2 = self.rgb_se_layer2(image_features_layer2) * image_features_layer2
        lidar_features_layer2 = self.lidar_se_layer2(lidar_features_layer2) * lidar_features_layer2
        image_features_layer2 = F.interpolate(
            image_features_layer2, scale_factor=4, mode='bilinear')
        lidar_features_layer2 = F.interpolate(
            lidar_features_layer2, scale_factor=4, mode='bilinear')
        ...
        image_features_layer3 = self.rgb_se_layer3(image_features_layer3) * image_features_layer3
        lidar_features_layer3 = self.lidar_se_layer3(lidar_features_layer3) * lidar_features_layer3
        image_features_layer3 = F.interpolate(
            image_features_layer3, scale_factor=2, mode='bilinear')
        lidar_features_layer3 = F.interpolate(
            lidar_features_layer3, scale_factor=2, mode='bilinear')
        ...
        image_features_layer4 = self.rgb_se_layer4(image_features_layer4) * image_features_layer4
        lidar_features_layer4 = self.lidar_se_layer4(lidar_features_layer4) * lidar_features_layer4
        ...
        image_features = image_features.view(
            bz, self.config.n_views * self.config.seq_len, -1)
...
class TransFuserPentaFusionSE(nn.Module):
    '''
