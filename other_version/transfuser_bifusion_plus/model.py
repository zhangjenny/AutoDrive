...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer, 
        ...
        self.enhance = SENet(channels=n_embd)
    ...
    def forward(self, image_tensor, lidar_tensor, velocity):
        ...
        image_tensor_bifusion = image_tensor.view(bz, -1, h, w)
        lidar_tensor_bifusion = lidar_tensor.view(bz, -1, h, w)

        token_embeddings_bifusion = self.bifusion(lidar_tensor_bifusion, image_tensor_bifusion)
        token_embeddings_bifusion = token_embeddings_bifusion.view(bz, -1, self.n_embd)
        ...
        token_embeddings = torch.cat([token_embeddings, token_embeddings_bifusion], dim=1)

        token_embeddings = self.enhance(token_embeddings) + token_embeddings
...
class TransFuserBiFusionPlus(nn.Module):
    '''
...
model = TransFuserBiFusionPlus(config, torch.device('cpu'))
