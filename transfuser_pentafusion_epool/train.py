...
from model import TransFuserPentaFusionNoPool
...
parser.add_argument('--id', type=str, default='transfuser_pentafusion_nopool', help='Unique experiment identifier.')
...
class Engine(object):
	...
	def train(self):
		...
		for data in tqdm(dataloader_train):
			...
			mse = float(mean_squared_error(pred_wp.detach().cpu().numpy().reshape(pred_wp.shape[0], -1), gt_waypoints.detach().cpu().numpy().reshape(gt_waypoints.shape[0], -1)).mean())
			r2_score_value = float(r2_score(pred_wp.detach().cpu().numpy().reshape(pred_wp.shape[0], -1), gt_waypoints.detach().cpu().numpy().reshape(gt_waypoints.shape[0], -1)).mean())
			mse_epoch += mse
			r2_score_epoch += r2_score_value

			num_batches += 1
			optimizer.step()

			writer.add_scalar('train_loss', loss.item(), self.cur_iter)
			writer.add_scalar('train_mse', mse, self.cur_iter)
			writer.add_scalar('train_r2_score', r2_score_value, self.cur_iter)
			self.cur_iter += 1
		
		
		loss_epoch = loss_epoch / num_batches
		mse_epoch = mse_epoch / num_batches
		r2_score_epoch = r2_score_epoch / num_batches
		self.train_loss.append(loss_epoch)
		self.train_mse.append(mse_epoch)
		self.train_r2_score.append(r2_score_epoch)
		self.cur_epoch += 1
	...
	def validate(self):
		...
		with torch.no_grad():	
			...
			for batch_num, data in enumerate(tqdm(dataloader_val), 0):
				...
				mse_epoch += float(mean_squared_error(pred_wp.detach().cpu().numpy().reshape(pred_wp.shape[0], -1), gt_waypoints.detach().cpu().numpy().reshape(gt_waypoints.shape[0], -1)).mean())
				r2_score_epoch += float(r2_score(pred_wp.detach().cpu().numpy().reshape(pred_wp.shape[0], -1), gt_waypoints.detach().cpu().numpy().reshape(gt_waypoints.shape[0], -1)).mean())
				

				num_batches += 1
					
			wp_loss = wp_epoch / float(num_batches)
			wp_mse = mse_epoch / float(num_batches)
			wp_r2_score = r2_score_epoch / float(num_batches)
            
			tqdm.write(f'Epoch {self.cur_epoch:03d}, Batch {batch_num:03d}:' + f' Wp_loss: {wp_loss:3.3f}'+ f' Wp_mse: {wp_mse:3.3f}'+ f' Wp_r2_score: {wp_r2_score:3.3f}')

			writer.add_scalar('val_loss', wp_loss, self.cur_epoch)
			writer.add_scalar('val_mse', wp_mse, self.cur_epoch)
			writer.add_scalar('val_r2_score', wp_r2_score, self.cur_epoch)
			
			self.val_loss.append(wp_loss)
			self.val_mse.append(wp_mse)
			self.val_r2_score.append(wp_r2_score)
	...
	def save(self):
		...
		log_table = {
			...
			'val_r2_score': self.val_r2_score,
		}
...
# Model
model = TransFuserPentaFusionNoPool(config, args.device)
