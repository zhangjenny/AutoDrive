...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k = self.key(left).view(B, T//2, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        q = self.query(right).view(B, T//2, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(right).view(B, T//2, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, attn_pdrop, resid_pdrop):
        super().__init__()
        self.ln1_left = nn.LayerNorm(n_embd)
        self.ln2_left = nn.LayerNorm(n_embd)
        self.attn_left = SelfAttention(n_embd, n_head, attn_pdrop, resid_pdrop)
        ...
        self.ln1_right = nn.LayerNorm(n_embd)
        self.ln2_right = nn.LayerNorm(n_embd)
        self.attn_right = SelfAttention(n_embd, n_head, attn_pdrop, resid_pdrop)
    ...
    def forward(self, x):
        ...
        # left block
        left = x[:,:T//2, :] + self.attn_left(self.ln1_left(x))
...
class Encoder_pro(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.image_encoder = ConvNeXt(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768])
        pretrain_url = 'https://dl.fbaipublicfiles.com/convnext/convnext_small_1k_224_ema.pth'
        checkpoint = torch.hub.load_state_dict_from_url(url=pretrain_url, map_location="cpu", check_hash=True)
        self.image_encoder.load_state_dict(checkpoint["model"])
        self.lidar_encoder = ConvNeXt(in_chans=2, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768])


        self.transformer1 = GPT(n_embd=96,
                            ...
                            config=config)
        self.transformer2 = GPT(n_embd=192,
                            n_head=config.n_head, 
                            block_exp=config.block_exp, 
                            n_layer=config.n_layer, 
                            vert_anchors=config.vert_anchors, 
                            ...
                            resid_pdrop=config.resid_pdrop,
                            config=config)
        self.transformer3 = GPT(n_embd=384,
                            n_head=config.n_head, 
                            block_exp=config.block_exp, 
                            n_layer=config.n_layer, 
                            vert_anchors=config.vert_anchors, 
                            ...
                            resid_pdrop=config.resid_pdrop,
                            config=config)
        self.transformer4 = GPT(n_embd=768,
                            n_head=config.n_head, 
                            block_exp=config.block_exp, 
                            n_layer=config.n_layer, 
                            vert_anchors=config.vert_anchors, 
    ...
    def forward(self, image_list, lidar_list, velocity):
        ...
        image_features = self.image_encoder.forward_features_layer_by_layer(0, image_tensor)
        lidar_features = self.lidar_encoder.forward_features_layer_by_layer(0, lidar_tensor)
        ...
        image_features = self.image_encoder.forward_features_layer_by_layer(1, image_features)
        lidar_features = self.lidar_encoder.forward_features_layer_by_layer(1, lidar_features)
        ...
        image_features = self.image_encoder.forward_features_layer_by_layer(2, image_features)
        lidar_features = self.lidar_encoder.forward_features_layer_by_layer(2, lidar_features)
        ...
        image_features = self.image_encoder.forward_features_layer_by_layer(3, image_features)
        lidar_features = self.lidar_encoder.forward_features_layer_by_layer(3, lidar_features)
        # fusion at (B, 512, 8, 8)
        ...
        image_features = self.image_encoder.norm(image_features.mean([-2, -1]))
        # image_features = torch.flatten(image_features, 1)
        image_features = image_features.view(bz, self.config.n_views * self.config.seq_len, -1)
        lidar_features = self.lidar_encoder.norm(lidar_features.mean([-2, -1]))
...
class TransFuserCross(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.encoder = Encoder_pro(config).to(self.device)
...
# model = TransFuserZ(config, torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    rgb = [torch.randn(1, 3, 256, 256)]
    lidar = [torch.randn(1, 2, 256, 256)]
    model = TransFuserCross(config, torch.device('cpu'))
